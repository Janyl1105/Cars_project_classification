...
#build_annotation_frame() — собирает таблицу с путями и bbox из devkit
...
#crop_annotation_frame() — обрезает изображения по bbox и сохраняет в cropped_dir, возвращает новый DataFrame с путями к обрезанным изображениям и остальными данными
...
#add_type_color_labels() — добавляет метки типа/цвета
...
#add_splits() — делает train/val/test split
def add_splits(
    frame: pd.DataFrame,
...
#prepare_cropped_dataset() — объединяет всё это в один pipeline: принимает dataset_root, cropped_dir, csv_path, запускает весь процесс и сохраняет итоговый CSV
